export default function StatsCards({ stats }) {
  const cards = [
    ...
    { label: 'Needs review', value: stats?.needs_review ?? '—', color: stats?.needs_review > 0 ? 'text-amber-700' : 'text-gray-900', amber: stats?.needs_review > 0 },
  ...
  return (
    <div className="flex items-center gap-4 px-6 py-4">
      {cards.map(card => (
        <div key={card.label} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border ${card.amber ? 'bg-amber-50 border-amber-200' : card.highlight ? 'bg-bb-green-light border-bb-green/30' : 'bg-white border-bb-border'}`}>
